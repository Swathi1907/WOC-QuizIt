...
class MainFragment : Fragment(R.layout.activity_main) {
    ...
    override fun onViewCreated(view:View,savedInstanceState: Bundle?) {
        ...
        val btnPickPdf = view.findViewById<Button>(R.id.btnPickPdf)
        val btnFetchUrl = view.findViewById<Button>(R.id.btnFetchUrl)
        val etUrl = view.findViewById<EditText>(R.id.etUrl)
        val btnExtractedText = view.findViewById<Button>(R.id.btnExtractedText)
        tvOutput = view.findViewById(R.id.tvOutput)
        btnGenerateAI = view.findViewById(R.id.btnGenerateAI)
        selectedCount = arguments?.getInt("count", -1)?:-1
        selectedDifficulty = arguments?.getString("difficulty")?:"medium"
        //quiztitle= intent.getStringExtra("Quiz Title")
        timeinminutes = arguments?.getInt("TIME_LIMIT",5)?:0
        if (selectedCount <= 0) {
            Toast.makeText(requireContext(), "Invalid question count", Toast.LENGTH_SHORT).show()
            return
        }
...
//
            ...
            Toast.makeText(requireContext(),"Please wait while we process your request...",Toast.LENGTH_LONG).show()
        ...
        btnExtractedText.setOnClickListener {
            if(extractedText.isBlank()) {
                Toast.makeText(requireContext(), "No Content Extracted Yet", Toast.LENGTH_SHORT).show()
                return@setOnClickListener
...
//
  //              } catch (_: Exception) { // ignore
               // }
               // Toast.makeText(requireContext(),"Extracting content in the PDF",Toast.LENGTH_SHORT).show()
   ...
   // }
    ...
    private fun copyUriToTempFile(uri: Uri): File {
        val temp = File.createTempFile("picked_pdf_", ".pdf", requireContext().cacheDir)
        requireContext().contentResolver.openInputStream(uri)?.use { input -> // asks who owns this uri that app sends data as a stream
        ...
        lifecycleScope.launch(Dispatchers.IO) { // coroutine automatically cancels when activity is destroyed
            try {
                ...
                withContext(Dispatchers.Main) {
                    ...
                    Toast.makeText(requireContext(),"URL Text Extracted", Toast.LENGTH_SHORT).show()
                }
            } catch (e: Exception) {
                withContext(Dispatchers.Main) {
